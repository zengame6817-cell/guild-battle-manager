...
function doPost(e) {
  ...
  try {
    ...
    if (body.action === "analyzeImageJob") {
      ...
      setAnalysisJob_(jobId, { status: "processing", startedAt: Date.now() });

      try {
        const data = analyzeTokyoDebunkerImage_(body.imageData, body.filename);
      ...
      // ブラウザ側ではこのPOSTレスポンスは読み取らず、
      // analysisResultをJSONPでポーリングする。
      return jsonTextResponse_({ ok: true });
    }
...
function getAnalysisJobResult_(jobId) {
  ...
  if (job.status === "error") {
    return { ok: true, status: "error", error: job.error || "画像解析に失敗しました。" };
  }
