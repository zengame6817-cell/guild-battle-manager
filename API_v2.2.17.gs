  ...
  try {
    ...
    switch (action) {
      ...
      case "savePowerImport":
        ...
        break;
      case "saveGuildEditor":
        result = saveGuildEditor_(String(p.guild || ""), decodePayload_(p.payload));
        break;
      case "analysisResult":
        result = getAnalysisJobResult_(String(p.jobId || ""));
        break;
      case "saveGuildResult":
        result = getSaveGuildJobResult_(String(p.jobId || ""));
        break;
      case "aiStatus":
        result = {
          ok: true,
      ...
      case "ping":
        result = {
          ok: true,
          message: "API v2.2.17 is working",
          version: getVersion_(),
...
function doPost(e) {
  ...
  try {
    ...
    if (body.action === "saveGuildEditorJob") {
      if (!jobId || !/^[A-Za-z0-9_-]{8,100}$/.test(jobId)) {
        throw new Error("保存ジョブIDが不正です。");
      }

      setSaveGuildJob_(jobId, {
        status: "processing",
        startedAt: Date.now()
      });

      try {
        const decoded = body.payload ? JSON.parse(body.payload) : {};
        const data = saveGuildEditor_(String(body.guild || ""), decoded);
        setSaveGuildJob_(jobId, {
          status: "done",
          finishedAt: Date.now(),
          data: data
...
function setSaveGuildJob_(jobId, value) {
  CacheService.getScriptCache().put(
    "GBM_SAVE_" + jobId,
    JSON.stringify(value),
    600
...
function getSaveGuildJobResult_(jobId) {
  if (!jobId || !/^[A-Za-z0-9_-]{8,100}$/.test(jobId)) {
    return { ok: false, error: "保存ジョブIDが不正です。" };
  }

  const raw = CacheService.getScriptCache().get("GBM_SAVE_" + jobId);
  if (!raw) {
    return { ok: true, status: "pending" };
  }
  ...
  const job = JSON.parse(raw);
  if (job.status === "done") {
    return { ok: true, status: "done", data: job.data };
  }
  if (job.status === "error") {
    return { ok: true, status: "error", error: job.error || "保存に失敗しました。" };
  }
  return { ok: true, status: job.status || "processing" };
}
...
function saveGuildEditor_(guildName, payload) {
  guildName = validateGuildName_(guildName);
  payload = payload || {};
  const rows = Array.isArray(payload.rows) ? payload.rows : [];
  const deletePlayers = Array.isArray(payload.deletePlayers) ? payload.deletePlayers : [];
  ...
  const currentNames = sheet.getRange(start, nameCol, end - start + 1, 1).getDisplayValues().flat();
  const rowByName = new Map();
  currentNames.forEach((v, i) => {
    const n = normalizeName_(v);
    if (n && !rowByName.has(n)) rowByName.set(n, start + i);
  });

  let deleted = 0;
  deletePlayers.forEach(name => {
    const row = rowByName.get(normalizeName_(name));
    if (!row) return;
    sheet.getRange(row, nameCol, 1, width).clearContent();
    rowByName.delete(normalizeName_(name));
    deleted++;
  });

  SpreadsheetApp.flush();

  const namesAfterDelete = sheet.getRange(start, nameCol, end - start + 1, 1).getDisplayValues().flat();
  const occupied = new Map();
  namesAfterDelete.forEach((v, i) => {
    const n = normalizeName_(v);
    if (n && !occupied.has(n)) occupied.set(n, start + i);
  });
  ...
  rows.forEach(item => {
    ...
    const entries = Array.isArray(item.entries) ? item.entries.slice(0, 3) : [];
    const normalized = normalizeName_(playerName);
    let row = occupied.get(normalized);

    if (!row) {
      const valuesNow = sheet.getRange(start, nameCol, end - start + 1, 1).getDisplayValues().flat();
      const emptyIndex = valuesNow.findIndex(v => !String(v || "").trim());
      if (emptyIndex < 0) throw new Error("空き行がありません。");
      row = start + emptyIndex;
      created++;
      occupied.set(normalized, row);
    } else {
      updated++;
    }
    ...
    sheet.getRange(row, nameCol, 1, width).setValues([vals]);
  });

  SpreadsheetApp.flush();
  const version = bumpVersion_();
  return { ok: true, updated, created, deleted, version };
}
...
function addGuildToList_(name) {
  name = validateGuildName_(name);
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(APP_CONFIG.guildListSheet);
  if (!sheet) throw new Error("プルダウン用シートが見つかりません。");

  const start = APP_CONFIG.guildListStartRow || 2;
  const end = APP_CONFIG.guildListEndRow || 200;
  const col = APP_CONFIG.guildListColumn || 11; // K列（Z用）
  const range = sheet.getRange(start, col, end - start + 1, 1);
  const values = range.getDisplayValues().flat();

  const normalized = normalizeName_(name);
  if (values.some(v => normalizeName_(v) === normalized)) return false;

  const emptyIndex = values.findIndex(v => !String(v || "").trim());
  if (emptyIndex < 0) throw new Error("プルダウン用K列に空きがありません。");

  sheet.getRange(start + emptyIndex, col).setValue(name);
  return true;
}
...
function analyzeTokyoDebunkerImage_(imageData, filename) {
  ...
  const prompt = [
    ...
    "日本語単位は実数の整数戦力へ変換する。例: 1610万=16100000、430万=4300000、5.2万=52000、8397=8397。アプリ側で万単位へ切り捨て表示するため、画像に書かれた値は正確に返す。",
